...
\begin{mybox}[證明2]{4.5em}{1em}{0.5em}
    ...
    \end{equation*}
    對於通過$K$點的任一直線，在直線上任取兩個相異點$X$和$X'$，其中$K\notin\left\{X,X'\right\}$，定義一個函數$g_K\left(X,X'\right)$：
    $$
        g_K\left(X,X'\right)=\begin{cases}
            0,&\text{若$K$是$XX'$的外分點}\\
            1,&\text{若$K$是$XX'$的內分點}
        \end{cases}
    $$
    ...
    $$
        ...
        =\measuredangle{\overrightarrow{AKP}}+g_K\left(A,A'\right)\pi+g_K\left(P,P'\right)\pi
    $$
    所以有
    $$
        \sin{\measuredangle{\overrightarrow{A'KP'}}}=(-1)^{g_K\left(A,A'\right)+g_K\left(P,P'\right)}\sin{\measuredangle{\overrightarrow{AKP}}}
    $$
    同理可得
    \begin{align*}
        \sin{\measuredangle{\overrightarrow{P'KB'}}}&=(-1)^{g_K\left(P,P'\right)+g_K\left(B,B'\right)}\sin{\measuredangle{\overrightarrow{PKB}}}\\
        \sin{\measuredangle{\overrightarrow{A'KQ'}}}&=(-1)^{g_K\left(A,A'\right)+g_K\left(Q,Q'\right)}\sin{\measuredangle{\overrightarrow{AKQ}}}\\
        \sin{\measuredangle{\overrightarrow{Q'KB'}}}&=(-1)^{g_K\left(Q,Q'\right)+g_K\left(B,B'\right)}\sin{\measuredangle{\overrightarrow{QKB}}}
    \end{align*}
    因此有
    \begin{align*}
        \frac{\sin{\measuredangle{\overrightarrow{A'KP'}}}/\sin{\measuredangle{\overrightarrow{P'KB'}}}}
        {\sin{\measuredangle{\overrightarrow{A'KQ'}}}/\sin{\measuredangle{\overrightarrow{Q'KB'}}}}
        &=\frac{(-1)^{g_K\left(A,A'\right)-g_K\left(P,P'\right)-g_K\left(P,P'\right)+g_K\left(B,B'\right)}}{(-1)^{g_K\left(A,A'\right)-g_K\left(Q,Q'\right)-g_K\left(Q,Q'\right)+g_K\left(B,B'\right)}}
...
\begin{mybox}[定理3]{4.5em}{1em}{0.5em}
    線-圓射影不變性：在任意圓$\Gamma$上任取五個相異點$K$、$A'$、$B'$、$P'$、$Q'$，
    任取直線$\ell$，設$\ell$分別與直線$KA'$、$KB'$、$KP'$和$KQ'$相交於$A$、$B$、$P$和$Q$四點，
    則有以下關係式成立：
    $$
        \left[A',B';P',Q'\right]=\left[A,B;P,Q\right]
    $$
...
\begin{mybox}[證明3]{4.5em}{1em}{0.5em}
    直接由定理1及正弦定理可以推導出
    \begin{align}
        \left[A,B;P,Q\right]&=\frac{\sin{\angle{AKP}}/\sin{\angle{PKB}}}
        {\sin{\angle{AKQ}}/\sin{\angle{QKB}}}\tag{定理1}\\
        &=\frac{\sin{\angle{A'KP'}}/\sin{\angle{P'KB'}}}
        {\sin{\angle{A'KQ'}}/\sin{\angle{Q'KB'}}}\notag\\
        &=\frac{A'P'/P'B'}{A'Q'/Q'B'}\tag{正弦定理}\\
        &=\left[A',B';P',Q'\right]\notag
    \end{align}
\end{mybox}
\begin{mybox}[定理4]{4.5em}{1em}{0.5em}
    （定理3的極限情況）：在任意圓$\Gamma$上任取四個相異點$A'$、$B'$、$P'$、$Q'$，任取直線$\ell$，
